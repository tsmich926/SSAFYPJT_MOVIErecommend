    ...
    if credits_response.status_code == 200:
        ...
        for crew in credits_data['crew']:
            if crew['job']=='Director':
                if crew['id'] in director_data_set:
                    ...
                    idx={'model':'movies.director','pk':crew['id'],'fields':{'name':crew['name'],"profile_path":crew['profile_path']}}
        ...
        for crew in credits_data['cast']:
            if crew['id'] in cast_data_set:
                ...
                idx={'model':'movies.actor','pk':crew['id'],'fields':{'name':crew['name'],"profile_path":crew['profile_path'],"gender":crew['gender'] }}
...
# 영화 데이터를 JSON 파일로 저장
with open('final_movie_data.json', 'w', encoding='utf-8') as file:
    json.dump(movie_data_fields, file, indent=4, ensure_ascii=False)
